...
/* The OpaqueDynamicReflectors layer contains the center sphere which will use the dynamic cube map to reflect local dynamic
objects */
...
/* Rendering to a cube map requires six additional render target views, one for each face, and one additional depth/
stencil buffer. Therefore, we must override the D3DApp::CreateRtvAndDsvDescriptorHeaps method and allocate these additional
descriptors */
...
void DynamicCubeMapApp::Draw(const GameTimer& gt)
{
	...
	/* After we rendered the scene to the cube map, we set our main render targets and draw the scene as normal, but with
	the dynamic cube map applied to the center sphere */
	DrawSceneToCubeMap();
...
// Set the constant data for each cube map face
...
void DynamicCubeMapApp::BuildDescriptorHeaps()
{
	...
	/* Create the views after copies of the descriptor handles have been saved */
...
/* Generally, the cube map faces will have a different resolution than the main back buffer. Therefore, for rendering to
the cube map faces, we need a depth buffer with dimensions that matches the resolution of a cube map face. However, because
we render to the cube faces one at a time, we only need one depth buffer for the cube map rendering. We build an additional
depth buffer and DSV */
...
{
	/* Because rendering to each cube map face utilizes a different camera, each cube face needs its own set of
	PassConstants. This is easy enough, as we just increase our PassConstants count by six when we create our frame
	resources */
...
/* Draw the scene to each face of the cube map, but not including the center sphere; this means we just need to render the
opaque and sky layers to the cube map */
...
degress field of view angle (both vertically and horizontally). Then have the camera look down the positive x-axis,
negative x-axis, positive y-axis, negative y-axis, positive z-axis, and negative z-axis, and to take a picture of the scene
(excluding the object O) from each of these six viewpoints. To facilitate this, we generate six cameras, one for each face,
centered at the given position (x, y, z) */
