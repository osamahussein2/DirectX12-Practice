...
void InstancingAndCullingApp::Draw(const GameTimer& gt)
{
	...
	/* Bind all the scene materials and textures once per-frame, and the only per draw call resource we need to set is the
	structured buffer with the instanced data */
...
{
	/* Note that we no longer have a per-object constant buffer. The per-object data comes from the instance buffer. Observe
	also how we use dynamic indexing to associate a different material for each instance, and a different texture. We are
	able to get quite a lot of per-instance variety in a single draw call */
