...
void PickingApp::Pick(int sx, int sy)
{
	...
	// Fill out the remaining render-item properties based on the selected triangle
	...
	for (auto ri : mRitemLayer[(int)RenderLayer::Opaque])
	{
		...
		// The picking ray is transformed from view space to the local space of an object
		...
		/* If the ray intersects one of the triangles, then it must have hit the mesh the triangle belongs to. Otherwise,
		the ray misses the mesh. Typically, we want the nearest triangle intersection, as it is possible for a ray to
		intersect several mesh triangles if the triangles overlap with respect to the ray */

		// Iterate through each triangle in the mesh and perform ray/triangle intersection test
