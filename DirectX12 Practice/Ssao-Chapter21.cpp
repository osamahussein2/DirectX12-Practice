...
//    //
//
//    /* When we render the scene view space normals, we also build the depth buffer for the scene. Consequently, when we
//    render the scene the second time with the SSAO map, we modify the depth comparison test to "EQUALS." This prevents any
//    overdraw in the second rendering pass, as only the nearest visible pixels will pass this depth comparison test.
//    Moreover, the second rendering pass does not need to write to the depth buffer because we already wrote the scene to the
//    depth buffer in the normal render target pass */
