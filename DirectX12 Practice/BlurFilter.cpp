...
/* The method caches the handles for all the descriptors it needs and then creates the corresponding descriptors. The reason
it caches the handles is so that it can recreate the descriptors when the resources change, which happens when the screen is
resized */
void BlurFilter::BuildDescriptors(CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuDescriptor, CD3DX12_GPU_DESCRIPTOR_HANDLE hGpuDescriptor,
	UINT descriptorSize)
{
...
/* Figure out how many thread groups to dispatch in each direction, and kick off the actual blur operation on the compute
shader */
void BlurFilter::Execute(ID3D12GraphicsCommandList* cmdList, ID3D12RootSignature* rootSig, ID3D12PipelineState* horzBlurPSO,
...
void BlurFilter::BuildResources()
{
	...
	// Create a UAV to a texture resource
