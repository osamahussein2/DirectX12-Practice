	...
	DXGI_FORMAT format)
{
	...
	/* Because the cube map faces will have a different resolution than the main back buffer, we need to define a new
	viewport and scissor rectangle that covers a cube map face */
...
// The descriptor handles are passed into the CubeRenderTarget::BuildDescriptors method which saves a copy of the handles
...
void CubeRenderTarget::BuildDescriptors()
{
	...
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE; // Sample the cubemap in a pixel shader
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		// Create a render target view to each element in the cube map texture array
		...
		// Render onto each cube map face one by one
...
void CubeRenderTarget::BuildResource()
{
	...
	/* Creating a cube map texture is done by creating a texture array with six elements (one for each face). Because we are
	going to render to the cube map, we must set the D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET flag. Build the cube map
	resource */
