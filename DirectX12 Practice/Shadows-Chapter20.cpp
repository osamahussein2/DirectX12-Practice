...
//    };
//
//    /* Observe that we set a null render target, which essentially disables color writes. This is because when we render the
//    scene to the shadow map, all we care about is the depth values of the scene relative to the light source. Graphics cards
//    are optimized for only drawing depth; a depth only render pass is significantly faster than drawing color and depth. The
//    active pipeline state object must also specify a render target count of 0 */
//
