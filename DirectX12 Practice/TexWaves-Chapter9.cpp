...
//{
//	/* To scroll a water texture over the water geometry, we translate the texture coordinates in the texture plane as a
//	function of time in this method, which gets called every update cycle. Provided the displacement is small for each frame,
//	this gives the illusion of a smooth animation. We use the wrap address mode along with a seamless texture so that we can
//	seamlessly translate the texture coordinates around the texture space plane */
//
//	// Scroll the water material texture coordinates.
//	auto waterMat = mMaterials["water"].get();
//
//	// Build and set water matrix
//	float& tu = waterMat->MatTransform(3, 0);
//	float& tv = waterMat->MatTransform(3, 1);
//
//	// Calculate the offset vector for the water texture
...
//
//	/* To tile the texture, we specify the wrap address mode and scale the texture coordinates by 5 using a texture
//	transformation matrix. Thus the texture coordinates are mapped to the domain [0, 5]^2 so that the texture is tiled 5 × 5 
